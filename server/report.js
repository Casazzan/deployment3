...
async function getServingsUsed(startDate, endDate) {
  ...
  for (let i = 0; i < query_res.rowCount; i++) {
    ...
    parseParallelStrings(
      row.side_ingredients,
      row.side_amt_servings,
      servingsMap
    );
    parseParallelStrings(
      row.appetizer_ingredients,
      row.appetizer_amt_servings,
      servingsMap
    );
  }
  return servingsMap;
  ...
  for (i = 0; i < restockList.length; i++) {
    ...
    if (!servingsMap.has(item.item_name)) {
      ...
      excessItemsList.push({
        item_name: item.item_name,
        restock_quantity: item.restock_quantity,
        servings_used: servingsMap.get(item.item_name),
      });
    }
  }
...
router.get("/restock", (req, res) => {
  ...
  pool.query(query).then((query_res) => {
    for (let i = 0; i < query_res.rowCount; i++) {
      ...
      if (item.servings < item.minimum_amount) items.push(item);
    }
    res.send(items);
  });
});
