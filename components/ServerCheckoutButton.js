...
var callAPIAsyncGetPrice = async (dishId, idString) => {
  ...
  const promise = fetch(
    `https://panda-express-deployment-3.onrender.com/dish_list/price?dish_id=${dishId}${idString}`
  );
...
var addToOrderHistory = async (dishId, idString) => {
  //console.log((await (await fetch(`https://panda-express-deployment-3.onrender.com/dish_list/price?dish_id=${dishId}${idString}`)).json()));
  const promise = fetch(
    `https://panda-express-deployment-3.onrender.com/order_history/price?dish_id=${dishId}${idString}`
  );
};
...
const orderTheItems = async (MyListOfOrders) => {
  for (var i = 0; i < MyListOfOrders.length; i++) {
    ...
    var nextID = (
      await (
        await fetch(
          "https://panda-express-deployment-3.onrender.com/order_history/nextID"
        )
      ).json()
    ).nextID;
    var price = await returnPrice(
      JSON.parse(localStorage.getItem("CurrentOrder"))
    );
    var date = "2022-10-25";
    console.log(
      `Order Sending: transaction_id=${nextID}&employee_id=0&date=${date}&type_of_dish=${dishtype}&entree_dish=${entreesResultString}&entree_amt_servings=${entreesAmountResultString}&side_ingredients=${sidesResultString}&side_amt_servings=${sidesAmountResultString}&appetizer_ingredients=${appetizersResultString}&appetizer_amt_servings=${appetizersAmountResultString}&price=${price}`
    );
    await fetch(
      `https://panda-express-deployment-3.onrender.com/order_history/add?transaction_id=${nextID}&employee_id=-1&date=${date}&type_of_dish=${dishtype}&entree_dish=${entreesResultString}&entree_amt_servings=${entreesAmountResultString}&side_ingredients=${sidesResultString}&side_amt_servings=${sidesAmountResultString}&appetizer_ingredients=${appetizersResultString}&appetizer_amt_servings=${appetizersAmountResultString}&price=${price}`
    );
    console.log("Sent");
  }
};
...
class ServerCheckoutButton extends Component {
  ...
  render() {
    return (
      ...
      <div
        class="ServerCheckoutButton"
        id="ServerCheckoutText"
        onClick={() => {
          orderTheItems(JSON.parse(localStorage.getItem("CurrentOrder")));
        }}
      >
        Checkout
      </div>
