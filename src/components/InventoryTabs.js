...
function InventoryTabs() {
  ...
  const updateServings = async () => {
    ...
    if (selected != "") {
      const promise = fetch(
        `https://panda-express-deployment-3.onrender.com/inventory/update_servings?id=${selected}&servings=${updateItemAmount}`
      );
      const response = await promise;
  ...
  const deleteItem = async () => {
    ...
    if (selected != "") {
      const promise = fetch(`https://panda-express-deployment-3.onrender.com/inventory/delete?id=${selected}`); 
      const response = await promise;
      setUpdateItemAmount(0);
    }
    */
  ...
  const addItem = async () => {
    const firstPromise = fetch(
      `https://panda-express-deployment-3.onrender.com/inventory/nextID`
    );
    ...
    const promise = fetch(
      `https://panda-express-deployment-3.onrender.com/inventory/add?id=${newID}&name=${name}&servings=${initial}&restock_quantity=${restock}&price=${price}&food_type=${type}&minimum_amount=${critical}`
    );
    const response = await promise;
    setName("");
    setPrice(0);
    ...
    <div className="container">
      ...
      <div className="content-tabs">
        ...
        >
          ...
          <p></p>
          <button className="SubmitButton" onClick={() => addItem()}>
        ...
        >
          ...
          <p></p>
          <button className="SubmitCritical">
            {" "}
            {translations[13].translatedText}{" "}
          </button>
          <p></p>
          <button className="SubmitCritical">
            {" "}
            {translations[14].translatedText}{" "}
          </button>
          <p></p>
          <hr />
          <h2> {translations[15].translatedText} </h2>
          <label> {translations[16].translatedText} </label>
          <InventorySelector rosterList={inventorySummary} />
          <p></p>
          ...
          <button
            ...
            id="update"
          >
            {translations[18].translatedText}
          </button>
          <button
            className="SubmitCritical"
            onClick={() => deleteItem()}
          ...
          >
            {" "}
            {translations[19].translatedText}{" "}
          </button>
        </div>
      </div>
    </div>
