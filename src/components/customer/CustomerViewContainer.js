...
class CustomerViewContainer extends React.Component {
  ...
  constructor(props) {
    ...
    var currentOrderFromLocalStorage = JSON.parse(
  ...
  returnPrice = async (MyListOfOrders) => {
    ...
    for (var i = 0; i < MyListOfOrders.length; i++) {
      ...
      if (dishtype == "bowl") {
        ...
        //it must be empty, add 0 and don't calculate further
        totalPrice += 0;
        continue;
      }
      ...
      if (timesRun == 0) {
        totalPrice =
          totalPrice + (await this.callAPIAsyncGetPrice(dish_id, ""));
      }
      if (timesRun > 0) {
        var resultString = "";
        if (timesRun > 1) {
        ...
        totalPrice =
          totalPrice + (await this.callAPIAsyncGetPrice(dish_id, resultString));
  ...
  render() {
    ...
    if (this.state.screen === "container")
      mainView = (
        <CustomerMenu
          createContainer={(ct) => this.createContainer(ct)}
      ...
      mainView = (
        <Container
          addToCart={(itemToAdd, itemType) =>
            this.addToCart(itemToAdd, itemType)
          }
          goBack={() => this.homeView()}
        />
      );
    // checkout
    else
      mainView = (
        <CustomerCheckout
          ...
          updateOrderCallback={(order) => this.updateOrder(order)}
        />
      );
    return (
